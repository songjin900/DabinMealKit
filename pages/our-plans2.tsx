...
********************************************************************************************************************************************************** */}
    ...
    const peopleNumClicked = (num: number) => {
        if (num) {
            setSelectedPeopleNum(num);
        }
    }

    const cuisineNumClicked = (num: number) => {
        if (num) {
            setSelectedCuisineNum(num);
        }
    }
                            ...
                            <div className={`flex flex-col w-full items-center justify-center `}>
                                    ...
                                    <div>
                                        {
                                            peopleNum
                                                .map((p) =>
                                                    <button className={`bg-white border-2 w-32 rounded-sm text-[#3A4884] 
                                                 ${selectedPeopleNum === p ? "font-bold border-2 border-[#3A4884]" : ""}`}
                                                        key={p} onClick={() => peopleNumClicked(p)}>{p}</button>)
                                        }
                                    </div>
                                    ...
                                    <div>
                                        {
                                            cuisineNum
                                                .map((p) =>
                                                    <button className={`bg-white border-2 w-24 rounded-sm text-[#3A4884] 
                                                    ${selectedCuisineNum === p ? "font-bold border-2 border-[#3A4884]" : ""}`}
                                                        key={p} onClick={() => cuisineNumClicked(p)}>{p}</button>)
                                        }
                                    </div>
                                ...
                                <div className={`border-2 flex flex-col w-[99%] gap-5 mt-5 p-3`}>
                                    <div className={`flex flex-col  text-sm`}>
                                        ...
                                        <span>{selectedCuisineNum} meals for {selectedPeopleNum} people per week</span>
                                        <span>{selectedCuisineNum * selectedPeopleNum} total servings</span>
                                    </div>
                                    <div className={`flex flex-col text-sm`}>
