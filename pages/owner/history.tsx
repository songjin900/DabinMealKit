...
const AllHistory: NextPage = () => {
    ...
    const { data } = useSWR(`/api/owner/history`);
    const [history, setHistory] = useState([]);
    const [orderId, setOrderId] = useState(-1);
    const [updateStatus, { loading: statusLoading, data: statusData }] = useMutation(`/api/owner/history`);
                                        ...
                                        <div key={order.id} className="border-2 rounded-xl space-x-4 bg-white mb-3 p-1">
                                            ...
                                            <div className="flex justify-between m-2" onClick={() => { setOrderId(order.id) }}>
                                                <div className="p-1 text-lg">
                                                    ...
                                                    {statusWithTimeSwitch(order?.deliveryStatus, order?.deliveryDate)}
                                                </div>
                                                <div className="flex">
                                                    {
                                                        orderId === order.id && (order.deliveryStatus === "Delivery Scheduled" || order.deliveryStatus === "Delivery Complete") ?
                                                            <div>
                                                                <DatePicker
                                                                    // selected={formattedDate(order.deliveryDate)}
                                                                    selected={new Date(order.deliveryDate)}
                                                                    onChange={(date: Date | null) => onDatePickerChange(date)}
